    ...
    <>
      <div className="w-full bg-cover min-h-[200px] bg-center bg-no-repeat mb-4 flex justify-center items-center relative before:w-full before:h-full before:absolute before:top-0 before-left-0 before:bg-opacity-50 before:bg-black z-0 rounded-tr rounded-tl overflow-hidden" style={{ backgroundImage: 'url(coverExample.jpg)' }}>
        <FileButton resetRef={resetRef} onChange={setFile} accept="image/png,image/jpeg">
          {(props) => <button className="bg-white relative z-1 text-ffbeb9 rounded-full p-4" {...props}><IconPhotoEdit color="#BF4565" /></button>}
        </FileButton>
      </div>
      <div className="relative w-28 -mt-16 left-1/2 -translate-x-1/2">
        <FileButton resetRef={resetRef} onChange={setFile} accept="image/png,image/jpeg">
          {(props) => <button {...props} className="w-full h-full rounded-full overflow-hidden border-2 border-white">
            ...
            <IconPhotoEdit className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white z-1 rounded-full p-2 text-BF4565" size={40} color="#BF4565" /></button>}
        </FileButton>
      </div>
      <div className="mt-0">
        <Textarea
      ...
      <div className="mt-2">
        <button className="w-full h-full flex text-center justify-center items-center rounded font-bold text-white text-base max-w-[100px] mx-auto min-h-[40px] bg-BF4565 pointer-events-none">Güncelle</button>
      </div>
    </>
  )
}
